...
from ..masklf import Ang_mask 
from ..CARM import CARM as AF
import math
import time
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        self.Ang_masks = nn.ModuleList([Ang_mask(embed_dims[0]),
                                        Ang_mask(embed_dims[1]),
                                        Ang_mask(embed_dims[2]),
                                        Ang_mask(embed_dims[3])])

        self.AFs = nn.ModuleList([
                    AF(channels=embed_dims[0]),
                    AF(channels=embed_dims[1]),
                    AF(channels=embed_dims[2]),
                    AF(channels=embed_dims[3])])
    ...
    def forward_features(self, List_Img):
        """
        x_rgb: B x N x H x W
        """
        x_rgb=List_Img[0]
        ...
        # B H*W/16 C
        _,N,C=x_rgb.shape
        List_Img = [self.extra_patch_embed1(items)[0] for items in List_Img]
        List_Img = [items.reshape(B, H, W, -1).permute(0, 3, 1, 2) for items in List_Img]
        x_row = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        x_e = self.Ang_masks[0](x_row,List_Img)
        x_e = x_e.permute(0,2,3,1).reshape(B,N,C)
        List_Img = [item.reshape(B, H, W, -1).permute(0, 3, 1, 2) for item in List_Img]
        ...
        x_rgb, x_e = self.AFs[0](x_rgb, x_e,2,1)
        ...
        _,N,C=x_rgb.shape
        List_Img = [self.extra_patch_embed2(items)[0] for items in List_Img]
        List_Img = [items.reshape(B, H, W, -1).permute(0, 3, 1, 2) for items in List_Img]
        x_row = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        x_e = self.Ang_masks[1](x_row,List_Img)
        x_e = x_e.permute(0,2,3,1).reshape(B,N,C)
        List_Img = [item.reshape(B, H, W, -1).permute(0, 3, 1, 2) for item in List_Img]
        ...
        x_rgb, x_e = self.AFs[1](x_rgb, x_e,2,1)
        ...
        _,N,C=x_rgb.shape
        List_Img = [self.extra_patch_embed3(items)[0] for items in List_Img]
        List_Img = [items.reshape(B, H, W, -1).permute(0, 3, 1, 2) for items in List_Img]
        x_row = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        x_e = self.Ang_masks[2](x_row,List_Img)
        x_e = x_e.permute(0,2,3,1).reshape(B,N,C)
        List_Img = [item.reshape(B, H, W, -1).permute(0, 3, 1, 2) for item in List_Img]
        ...
        x_rgb, x_e = self.AFs[2](x_rgb, x_e,2,1)
        ...
        _,N,C=x_rgb.shape
        List_Img = [self.extra_patch_embed4(items)[0] for items in List_Img]
        List_Img = [items.reshape(B, H, W, -1).permute(0, 3, 1, 2) for items in List_Img]
        x_row = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        x_e = self.Ang_masks[3](x_row,List_Img)
        x_e = x_e.permute(0,2,3,1).reshape(B,N,C)
        List_Img = [item.reshape(B, H, W, -1).permute(0, 3, 1, 2) for item in List_Img]
        ...
        x_rgb, x_e = self.AFs[3](x_rgb, x_e,2,1)
    ...
    def forward(self, List_Img):
        out = self.forward_features(List_Img)
        return out
